...
dotenv.config();
// dotenv check
console.log("Client Key: ".concat(process.env.CLIENT_KEY, ", Guild ID: ").concat(process.env.GUILD_ID, ", Token: ").concat(process.env.DISCORD_TOKEN));
...
var commandFolders = fs.readdirSync(join(__dirname, 'commands'));
// -> Read command files
for (var _i = 0, commandFolders_1 = commandFolders; _i < commandFolders_1.length; _i++) {
    var folder = commandFolders_1[_i];
    var commandFiles = fs.readdirSync(join(__dirname, 'commands', folder)).filter(function (file) { return file.endsWith('.ts'); });
    ...
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                ...
                return [4 /*yield*/, rest.put(Routes.applicationGuildCommands(process.env.CLIENT_KEY, process.env.GUILD_ID), { body: commands })];
            case 1:
                data = _a.sent();
